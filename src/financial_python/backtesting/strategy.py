...
class MovingAverageCrossoverStrategy(TradingStrategy):
    ...
    def generate_signals(self) -> pd.DataFrame:
        ...
        signals['short_mavg'] = self.data.rolling(window=self.short_window, min_periods=1).mean()
        signals['long_mavg'] = self.data.rolling(window=self.long_window, min_periods=1).mean()
